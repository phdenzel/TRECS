...
#   If you have donwloaded TRECS_Inputs on host, mount it as a docker volume instead;
#   if this is not possible (for whatever reason), uncomment the next two lines.
#RUN aria2c -x 10 --out TRECS_Inputs.tgz https://www.dropbox.com/s/crkzwho0hqc565g/TRECS_Inputs.tgz?dl=1
...
RUN --mount=type=ssh . /clone.sh TRECS git@github.com:phdenzel/TRECS.git 145ddd84ac6ee467b47600dc5a571ba336061fb1 \
    && rm -rf doc


# Build dependencies (for compatibility the same image as in final stage)
...
RUN cd /TRECS && make sampler_continuum
# RUN cd /TRECS && make all


# # Final stage
# FROM python:3.10.9-bullseye
# RUN addgroup --g 1000 trecs-group
# RUN adduser -u 1000 -G trecs-group -h /home/trecs -D trecs
# USER trecs
...
COPY sampler_hi/sampler.f90 /TRECS/sampler_hi/sampler.f90
COPY modules/sampler_io.f90 /TRECS/modules/sampler_io.f90
COPY bin/continuum.inp /TRECS/bin/continuum.inp
